...
{
    public static AccountListPageModel Model;
    ...
    public AccountListPageModel model
    {
        get => Model;
        set => Model = value;
    }
    ...
    protected override void OnNavigatedTo(NavigationEventArgs e)
    {
        ...
        try
        {
            model = e.Parameter as AccountListPageModel;
    ...
    private void CreateAccount(object sender, RoutedEventArgs e)
    {
        ...
        var screen =
            new AccountCreateDialog(dialog, newAccount, new List<Tag>(model.Tags.Select(uitag => uitag.Target)));
    ...
    private async void DeleteButton_OnDrop(object sender, DragEventArgs e)
    {
        if (e.DataView.Contains(StandardDataFormats.Text))
        {
            ...
            var id = long.Parse(await e.DataView.GetTextAsync());
            model.Accounts.ToList().ForEach(e => Console.WriteLine("list>" + e.Identifier.id));
            foreach (var account in model.Accounts)
                if (account.Identifier.id == id)
                {
                    var dialogOperation = new AskDialogOperation(this, "Delete Account", "Delete", "Cancel",
            ...
            foreach (var tag in model.Tags)
                ...
                {
                    var dialogOperation = new AskDialogOperation(this, "Delete tag", "Delete", "Cancel",
    ...
    private async void DeleteButton_OnDragEnter(object sender, DragEventArgs e)
    {
        long data = -1;
        e.AcceptedOperation =
            e.DataView.Contains(StandardDataFormats.Text) && long.TryParse(await e.DataView.GetTextAsync(), out data)
    ...
    private void StartEditAccount(object sender, RoutedEventArgs e)
    {
        ...
        model.Navigator.Navigate(uiAccount.Target.AccountEditor, operation,
    ...
    {
        if (Model.LatestException == null) return;
